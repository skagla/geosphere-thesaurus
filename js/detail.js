...
var detail = {
            ...
            if (data.results.bindings.length > 1) {
                ...
                div.append(`<hr>
                                ...
                                <i class="fas fa-caret-down caret"></i>
                                    <em id="detailsBtn" style="display:inline-block;">RDF statements</em>
                                </summary>
    ...
    provideAll: function (divID, uri, offset) { //provide all available concepts for navigation
        ...
        var query = `PREFIX dcterms:<http://purl.org/dc/terms/>
                    ...
                    SELECT DISTINCT ?s (STR(?l) AS ?Label) ?cs (STR(?csl) AS ?Schema) (COALESCE(?csd, "") AS ?SchemaDesc) (COALESCE(?sd, "") AS ?Desc) ?top
                    ...
                    BIND(IF(?s = ?tc, true, false) AS ?top)
        ...
        ws.json(ws.getProject(uri), query, "b", function (data) {
            ...
            $('#' + divID).append('');
            if (offset == 0) {
                $('#allConceptsHeader').html(data.results.bindings[0].Schema.value + ' (alphabetical list of concepts)');
